...
# valeurs npe limites optimales pour discriminer les neutrinos
npe_min = 4e3
npe_max = 6e4
...
for run in runs:
    ...
    for i in range(n_files):
        ...
        if run >= 4330:
            ...
            data = f'/storage/gpfs_data/juno/junofs/users/mcolomer/water_data/{run}/simple_out/TQ/tqcalib_output_RUN.{run}.JUNODAQ.WaterFull.ds-2.global_trigger_cotiwaverec_J25.1.3_{i+1000}.root'
        else:
    ...
    plt.tight_layout()
    plt.savefig(f"/storage/gpfs_data/juno/junofs/users/frosso/data_simu_20kpc_veto1_run{run}_{n_files - n_error}files.png", dpi=300)