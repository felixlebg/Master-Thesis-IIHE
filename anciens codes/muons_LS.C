...
void muons_LS() {
    ...
    TH1D* my_histo = new TH1D("myh","",100,0,30); // (bin, start, end) (start à 10 pour suppr le bruit de fond du graphe)

    // for loop over all the files

    for(int f=1000; f<3001; f++){


        int last_idx = -99;
        // set TString and TFile name (without the for loop bc we have only 1 file)
        TString oecname = TString::Format("/storage/gpfs_data/juno/junofs/users/mcolomer/J23_JUNO/muon/oec/wpo/muon-oec-J23.1.0.-rc1-OR-ed-%d.root", int(f));
        ...
        TString calib_treename = "Event/CdLpmtElec/CdLpmtElecEvt";
        TTree* calib_tree = (TTree*)(OECfile->Get(calib_treename));
        ...
        JM::OecEvt* calibobj = 0;
        calib_tree->SetBranchAddress("CdLpmtElecEvt", &calibobj);
        calib_tree->GetBranch("CdLpmtElecEvt")->SetAutoDelete(true);
