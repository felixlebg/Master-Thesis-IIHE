...
for i in range(n_files):
    outfile = open(f"/storage/gpfs_data/juno/junofs/users/frosso/out/simu/{filling}/out_simu{veto}_file{i+1}.txt", "w")
    ...
    cd_evt_l, wp_evt_divided = sub_list(root_simu_ccsn, file_dividing, dt_max_veto2)
    ...
    for j in range(len(cd_evt_l)):
        ...
        # veto 1
        if veto1 and abs(dt) < dt_max_veto1:
            continue
        ...
        if veto2:
            ...
            for wp_evt in wp_evt_divided[index]:  # coïncidences avec les muons de la waterpool
                wp_t = wp_evt[1]
                dt_wp = wp_t - t_evt
                if abs(dt_wp) < dt_max_veto2 and wp_evt[0] > 1000:
                    correl = True
                    break
