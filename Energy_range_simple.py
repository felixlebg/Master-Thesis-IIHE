...
from functions import Opt_E_range_simple
import sys
...
threshold_index = int(param[1])
filling = ["25%", "half", "75%"][filling_index]
distance = 60 # en kpc

number_of_files = 1000

threshold = [1e-2, 5e-3, 1e-3, 5e-4, 1e-4][threshold_index]
...
opt_range = Opt_E_range_simple(files_ccsn, files_data, threshold, number_of_bins, distance, runs, filling)
print(f"Filling {filling}, méthode simple")
print(f"Optimal range with max background rate of {threshold}: {opt_range}")
